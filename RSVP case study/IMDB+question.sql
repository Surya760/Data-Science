...
-- Result :
-- We know that count(*) will count null values whereas count(column_name) won't count null values .
-- Thus if we substract count(column_name) from count(*)  we would get if a column has null vals or not .The same question
-- can also be solved using this approach .
-- Thus we can see from o/p that columns country , worlwide_gross_income,languages and production_company have
-- null values .
...
-- March has got highest number of movies that is 824 

/*The highest number of movies is produced in the month of March.
So, now that you have understood the month-wise trend of movies, let’s take a look at the other details in the movies table. 
We know USA and India produces huge number of movies each year. Lets find the number of movies produced by USA or India for the last year.*/
  
SELECT count(id) as Movies_USA_India from movie where 
(( country LIKE '%INDIA%') OR
(country LIKE '%USA%' ))
AND 
year = 2019;

-- Here as we are asked to find movies for USA or India thus 
-- the result is 1059 movies 
...
-- Result is Drama with 4285 movies
...
FROM
    ...
    FROM
        genre
    GROUP BY movie_id
    HAVING COUNT(genre) = 1) AS genre_movie;
...
;
-- Thriller movies rank 3rd with a count of 1484 movies 
...
-- Result Dream Warrior Pictures or National Theatre Live or both rank the same i.e. at teh top 
-- It's ok if RANK() or DENSE_RANK() is used too
...
-- Around 361 movies were given a rating of 8 
...
with german_count as 
(SELECT 
    sum(total_votes) as German
FROM
    ratings 
        INNER JOIN
...
SELECT german_count.German,
	   Italian_count.Italian, 
       (CASE
			WHEN Italian_count.Italian > german_count.German THEN "Italian Vote Count is More than German"
            ELSE "German Vote Count is More than Italian"
       END
       ) as Result
 from german_count,italian_count;

-- German Vote count is pretty high as compred to Italian .
-- Answer is Yes
...
WHERE r.avg_rating > 8  
AND genre IN (SELECT genre from genre_top_3)
GROUP BY n.name
...
SELECT g.genre as genre,
        ...
        AVG(ROUND(AVG(m.duration),2)) OVER(ORDER BY g.genre ROWS 7 PRECEDING) AS moving_avg_duration
FROM genre AS g 
...
as (SELECT genre,
           ...
           DENSE_RANK() OVER (PARTITION BY year ORDER BY CAST(REPLACE(REPLACE(ifnull(worlwide_gross_income, 0), 'INR', ''), '$', '') as decimal(20)) desc) as movie_rank
    FROM movie m
        INNER JOIN genre g
				on m.id = g.movie_id
    WHERE genre in 
